...
export default function EmployeeOnboarding() {
  ...
  const [deletedEmployee, setDeletedEmployee] = useState<{ id: string; full_name: string } | null>(
    null
  )
  ...
  const handleSaveSection = async () => {
    ...
    try {
      if (activeSection === 'systemAccess') {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            address_door: v.address_door || null,
            address_street: v.address_street || null,
            address_area: v.address_area || null,
            address_city: v.address_city || null,
            address_pincode: v.address_pincode || null,
        ...
        const { error } = await supabase.from('employee_identity').upsert(
          {
            employee_id: id,
            college_name: v.college_name || null,
            course: v.course || null,
            study_year: v.study_year || null,
          },
        ...
        if (v.emergency_name) {
          const { error } = await supabase.from('employee_emergency_contacts').upsert(
            {
              employee_id: id,
              contact_name: v.emergency_name,
              relationship: v.emergency_relationship || null,
          ...
          const { error } = await supabase.from('employee_bank_details').upsert(
            {
              ...
              ifsc_code: v.ifsc_code || null,
              account_holder_name: v.account_holder_name || null,
      ...
      showToast('Error: ' + (e instanceof Error ? e.message : 'Failed to save'), 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleResetPassword = async () => {
    if (!supabaseAdmin) {
      setResetPwError('Admin API not configured (SERVICE_ROLE_KEY missing)')
      return
    }
    const v = watch()
    if (!v.reset_password || v.reset_password.length < 6) {
      setResetPwError('Minimum 6 characters')
      return
    }
    if (v.reset_password !== v.reset_confirm_password) {
    ...
    if (!existing?.auth_user_id) {
      ...
      const { data: authData, error: createErr } = await supabaseAdmin.auth.admin.createUser({
        email,
        password: v.reset_password,
        email_confirm: true,
        role: 'authenticated',
      })
      if (createErr) {
        setResetPwError('Failed to create auth account: ' + createErr.message)
      ...
      const { error: linkErr } = await supabase
        ...
        .eq('id', id)
      if (linkErr) {
        setResetPwError('Auth account created but failed to link: ' + linkErr.message)
        setResetPwSaving(false)
        return
      }
        ...
        <Card className="mb-4 border-yellow-300 bg-yellow-50">
          <CardContent className="p-4">
            <p className="text-sm font-medium text-foreground mb-2">
              A deleted record exists for this phone number:{' '}
              <strong>{deletedEmployee.full_name}</strong>
            </p>
            <p className="text-xs text-muted-foreground mb-3">
              Would you like to restore their details and update what has changed?
            </p>
            <div className="flex gap-2">
              <Button size="sm" onClick={handleRestoreDeleted}>
                Yes, Restore Details
              </Button>
              <Button variant="outline" size="sm" onClick={() => setDeletedEmployee(null)}>
                No, Enter Fresh
              </Button>
            </div>
          </CardContent>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Card>
            <CardContent className="p-6 space-y-4">
              <h2 className="text-lg font-semibold text-foreground">
                1. {t('employees.sections.systemAccess')}
              </h2>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    clearSectionError('full_name')
                  }}
                />
                {sectionErrors.full_name && (
                  <p className="text-destructive text-xs">{sectionErrors.full_name}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                {sectionErrors.phone && (
                  <p className="text-destructive text-xs">{sectionErrors.phone}</p>
                )}
                ...
                {isEdit && (
                  <p className="text-yellow-600 text-xs">
                    Changing phone number will update the login username
                  </p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {sectionErrors.role && (
                  <p className="text-destructive text-xs">{sectionErrors.role}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-4 mt-2">
                  <label className="flex items-center gap-2 cursor-pointer min-h-[48px]">
                    <input
                      ...
                      {...register('branch_kr')}
                      onChange={(e) => {
                        register('branch_kr').onChange(e)
                        clearSectionError('branch')
                      }}
                    />
                ...
                {sectionErrors.branch && (
                  <p className="text-destructive text-xs">{sectionErrors.branch}</p>
                )}
              </div>
                ...
                <>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {sectionErrors.password && (
                      <p className="text-destructive text-xs">{sectionErrors.password}</p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        clearSectionError('confirmPassword')
                      }}
                    />
                    {sectionErrors.confirmPassword && (
                      <p className="text-destructive text-xs">{sectionErrors.confirmPassword}</p>
                    )}
                  </div>
                </>
              )}
        ...
        {activeSection === 'personal' && (
          <Card>
            <CardContent className="p-6 space-y-4">
              <h2 className="text-lg font-semibold text-foreground">
                2. {t('employees.sections.personal')}
              </h2>
              ...
              <div className="space-y-1.5">
                <Label>
                  {t('employees.fields.email')} ({t('common.optional')})
                </Label>
              ...
              <div className="space-y-1.5">
                <Label>
                  {t('employees.fields.mapsUrl')} ({t('common.optional')})
                </Label>
        ...
        {activeSection === 'identity' && (
          <Card>
            <CardContent className="p-6 space-y-4">
              <h2 className="text-lg font-semibold text-foreground">
                3. {t('employees.sections.identity')}
              </h2>
              ...
              <h3 className="font-medium text-foreground">
                College Details ({t('common.optional')})
              </h3>
              <div className="space-y-1.5">
        ...
        {activeSection === 'emergency' && (
          <Card>
            <CardContent className="p-6 space-y-4">
              <h2 className="text-lg font-semibold text-foreground">
                4. {t('employees.sections.emergency')}
              </h2>
        ...
        {activeSection === 'work' && (
          <Card>
            <CardContent className="p-6 space-y-4">
              <h2 className="text-lg font-semibold text-foreground">
                5. {t('employees.sections.work')}
              </h2>

              <div className="space-y-1.5">
                <Label>
                  {t('employees.fields.experience')} ({t('common.optional')})
                </Label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>
                    {t('employees.fields.refName')} ({t('common.optional')})
                  </Label>
                  <Input {...register('reference_name')} />
                </div>
                <div className="space-y-1.5">
                  <Label>
                    {t('employees.fields.refPhone')} ({t('common.optional')})
                  </Label>
        ...
        {activeSection === 'bank' && (
          <Card>
            <CardContent className="p-6 space-y-4">
              <h2 className="text-lg font-semibold text-foreground">
                6. {t('employees.sections.bank')}
              </h2>
              <div className="bg-yellow-50 rounded-lg p-3 text-sm text-yellow-700">
                Bank details collected now — used in Phase 13 Payroll. Account number stored
                encrypted.
              </div>
              ...
              <div className="space-y-1.5">
                <Label>
                  {t('employees.fields.upiId')} ({t('common.optional')})
                </Label>
