  ...
  return (
    <PageContainer>
      ...
      <SectionCard
        ...
        description="Used by the Sales Reconciliation engine to calculate predicted revenue from closing stock weights."
        padding="none"
      >
        {isLoading && (
          <div className="p-4">
        ...
        {!isLoading && !error && latestByItem.length === 0 && (
          <div className="p-4">
            <EmptyState
              icon={Pencil}
              title="No weight configurations"
              description="No weight configurations found. Apply migration 005 first."
            />
          </div>
        )}
            ...
            <TableBody>
              {latestByItem.map((cfg) => {
                ...
                return (
                  ...
                  >
                    <TableCell className="font-medium">{cfg.item_master?.name_en ?? '—'}</TableCell>
