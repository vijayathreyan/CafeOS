...
const INPUT =
  'w-20 h-9 sm:w-24 sm:h-10 rounded-md border border-input bg-background text-center text-sm px-1 focus:outline-none focus:ring-2 focus:ring-ring'
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const inserts = entries
        .filter((e) => e.shift1 > 0 || e.shift2 > 0)
        .map((e) => ({
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-muted-foreground text-xs bg-muted/50">
            ...
            <th className="text-right py-2 font-semibold w-24 rounded-r">
              {t('postpaid.dailyTotal')}
            </th>
          </tr>
        </thead>
        <tbody>
